...
const doneProjects: ProjectCardData[] = projects.filter((p) =>
  ["MVP", "actively expanding", "maintenance", "handed over", "delivered"]
    .includes(p.status)
);
const ongoingProjects: ProjectCardData[] = projects.filter((p) =>
  ["WIP"].includes(p.status)
);
const futureProjects: ProjectCardData[] = projects.filter((p) =>
  ["sketch", "idea"].includes(p.status)
);
    ...
    <main className="max-w-4cl mx-auto sm:scroll-smooth">
      ...
      <div id="goalsSection" className="bg-slate-400 mb-2">
        <section className="max-w-4xl mx-auto p-4  sm:flex-row sm:justify-between resize">
          ...
          <p>
            I&apos;m looking for work that will let me showcase my full stack
            skills.
          </p>
          <p>
            Knowledge management and personal productivity are especially close
            to me.
          </p>
        </section>
        ...
        <section className="max-w-4xl mx-auto p-4  sm:flex-row sm:justify-between resize">
          ...
          <p>
            ...
            create a React Native app for my BSc research project
          </p>
          <p>
            <input type="checkbox" disabled checked />{" "}
            create some small web projects, like extensions
          </p>
          <p>
            <input type="checkbox" disabled checked /> graduate
          </p>
        </section>
        <hr className="mx-auto bg-dark dark:bg-white w-1/2" />
        <section className="max-w-4xl mx-auto p-4 sm:flex-row sm:justify-between resize">
          ...
          <p>
            to have most of my current projects completed, having transformed at
            least one of them into something a million people uses.
          </p>
          <p>
            I&apos;d be grateful if one of them became my main job, but that
            might not be in the cards.
          </p>
          <p>
            Working on only that that you want to work is a luxury. I plan to
            attain it.
          </p>
        </section>
      </div>
...
function CollectionProjects(props: { projects: ProjectCardData[] }) {
  if (props.projects.length == 0) {
    return (
      <div>
        <h3>
          if you&apos;re seeing this it means some screenshots are missing
        </h3>
      </div>
    );
  }
  return (
    <div className="grid grid-flow-row grid-cols-3  w-fit gap-2">
