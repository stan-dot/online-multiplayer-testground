...
export class Shape {
  ...
  constructor(points: number[][]) {
    this.points = points;
    const unsanitizedSegments: (SegmentChecker | null)[] = this.points.map((v: number[], index: number) => this.addSegmentChecker(v, index));
    this.segments = unsanitizedSegments.filter(segment => segment !== null) as SegmentChecker[];
  }
   ...
   */
  private addSegmentChecker(point: number[], index: number): SegmentChecker | null {
    let x1 = point[0];
    let y1 = point[1];
